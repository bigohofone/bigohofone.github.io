...
export function ExperienceRow({ item }) {
  return (
    <Dialog>
      <DialogTrigger asChild>
        ...
        >
          ...
          <p className="hidden text-right text-sm text-muted-foreground md:block md:flex-[4] md:min-w-0 md:truncate">{item.role}</p>
      ...
      <DialogContent className="sm:max-w-xl">
        <DialogHeader className="gap-5">
          ...
          <div className="space-y-1">
            ...
            <DialogDescription>
              {item.organization}{item.location && <span> · {item.location}</span>}
            </DialogDescription>
